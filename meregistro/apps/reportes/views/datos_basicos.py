...
@login_required
@credential_required('reportes_datos_basicos_unidades_servicio')
def unidades_servicio(request):

    sql = Reporte.get_sql_file_content('405_datos_basicos_unidades_servicio.sql').replace('{{AMBITO_PATH}}', "'" + str(request.get_perfil().ambito.path) + "%%'")

    cursor = connection.cursor()
    cursor.execute(sql)

    filename = 'unidades_servicio_datos_basicos_' + str(date.today()) + '.xls'
    reporte = Reporte(headers=[\
        'JURISDICCIÓN',\
        'GESTIÓN',\
        'TIPO UNIDAD DE SERVICIO',\
        'CUE',\
        'ESTABLECIMIENTO',\
        'TELÉFONO',\
        'EMAIL',\
    ...
    for row in Reporte.dictfetchall(cursor):
        reporte.rows.append([\
            row['jurisdiccion'].encode('utf8') if row['jurisdiccion'] else '',\
            row['gestion'].encode('utf8') if row['gestion'] else '',\
            row['tipo_unidad_servicio'].encode('utf8') if row['tipo_unidad_servicio'] else '',\
            row['cue'].encode('utf8') if row['cue'] else '',\
            row['establecimiento'].encode('utf8') if row['establecimiento'] else '',\
            row['telefono'].encode('utf8') if row['telefono'] else '',\
            row['email'].encode('utf8') if row['email'] else '',\

...
class ConsultaValidezFormFilters(forms.Form):
	...
	carrera_choices = [[c, c] for i,c in Titulo.objects.order_by('carrera').distinct('carrera').values_list('id', 'carrera')]
	titulo_choices = [[d, d] for i,d in Titulo.objects.order_by('denominacion').distinct('denominacion').values_list('id', 'denominacion')]

	jurisdiccion = forms.ChoiceField(choices=[['0', '--------']] + jur_choices, label='Jurisdiccion', required=False)

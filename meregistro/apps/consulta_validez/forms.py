	...
	def buildQuery(self, q=None):
		...
		if self.is_valid():
			...
			if filter_by('tipo_gestion'):
				...
				from django.db.models import Q
				q = q.filter(
					(Q(tipo_unidad_educativa='Sede') & Q(unidad_educativa_id__in=[e.pk for e in Establecimiento.objects.filter(dependencia_funcional__tipo_gestion_id=self.cleaned_data['tipo_gestion'])])) |
					(Q(tipo_unidad_educativa='Anexo') & Q(unidad_educativa_id__in=[a.pk for a in Anexo.objects.filter(establecimiento__dependencia_funcional__tipo_gestion_id=self.cleaned_data['tipo_gestion'])]))
				)
			if filter_by('cue'):

...
from apps.registro.models import DependenciaFuncional, Jurisdiccion, GestionJurisdiccion, TipoGestion, TipoDependenciaFuncional, TipoEducacion
...
class DependenciaFuncionalFormFilters(forms.Form):
    ...
    tipo_educacion = forms.ModelChoiceField(queryset = TipoEducacion.objects.order_by('descripcion'), label = 'Tipo de educación', required = False)
    ...
    def buildQuery(self, q = None):
        ...
        if filter_by('gestion_jurisdiccion'):
            q = q.filter(gestion_jurisdiccion = self.cleaned_data['gestion_jurisdiccion'])
        ...
        if filter_by('tipo_educacion'):
            q = q.filter(tipo_educacion = self.cleaned_data['tipo_educacion'])
        return q

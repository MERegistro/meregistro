...
class UsuarioFormFilters(forms.Form):
  ...
  def buildQuery(self, q = None):
    ...
    """
    if q is None: q = Usuario.objects.all()
    if self.is_valid():
      def filter_by(field):
        return self.cleaned_data.has_key(field) and self.cleaned_data[field] != '' and self.cleaned_data[field] is not None
      if filter_by('documento'): q = q.filter(documento__istartswith = self.cleaned_data['documento'])
      if filter_by('nombre'): q = q.filter(nombre__istartswith = self.cleaned_data['nombre'])
      if filter_by('apellido'): q = q.filter(apellido__istartswith = self.cleaned_data['apellido'])
      if filter_by('email'): q = q.filter(email__istartswith = self.cleaned_data['email'])
      if filter_by('tipo_documento'): q = q.filter(tipo_documento = self.cleaned_data['tipo_documento'])
    return q
